...
function runAddSubDemo(ctx, doneCb) {
    ...
    let cycle = () => {
        ...
        writeLabel(ctx, vals, `${a} ${s(b)} ${Math.abs(b)} = ${a+b} ≡ ${fmath.reduce(a+b)} (mod ${fmath.p})`);
        addSub(ctx, a, b, (c) => {
...
function runMultDemo(ctx, doneCb) {
    ...
    let a = r(), b = r();
    let cycle = () => {
        writeLabel(ctx, vals, `${a} ${TIMES_STR} ${b} = ${a*b} ≡ ${fmath.reduce(a*b)} (mod ${fmath.p})`);
        mult(ctx, a, b, (c) => {
            if (doneCb) doneCb(a, b, c);
            if (common.canvasIsScrolledIntoView(ctx.canvas)) {
                a = r();
...
function runNegateDemo(ctx, doneCb) {
    ...
    let cycle = () => {
        refresh();
        writeLabel(ctx, vals, `-${a} mod ${fmath.p} = ${negA}`);
        addSub(ctx, a, b, () => {
...
function runDivDemo(ctx, doneCb) {
    ...
    let cycle = () => {
        refresh();
        writeLabel(ctx, vals, `${a} ${TIMES_STR} ${b} = ${a*b} ≡ ` +
            `${fmath.reduce(a*b)} (mod ${fmath.p}), therefore ${a}${INV_STR} is ${b}`);
        mult(ctx, a, b, (c) => {
            if (doneCb) doneCb(a, b, c);
            if (common.canvasIsScrolledIntoView(ctx.canvas)) {
...
function runSqrtDemo(ctx, doneCb) {
    ...
    let cycle = () => {
        refresh();
        writeLabel(ctx, vals, `${b} ${TIMES_STR} ${b} = ${b*b} ≡ ` +
            `${fmath.reduce(b*b)} (mod ${fmath.p}), therefore ${b} is a square root of ${a}`);
        mult(ctx, b, b, () => {
