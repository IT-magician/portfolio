...
let drawAxisLines = (ctx, vals) => {
    ...
    // 동적으로 눈금 생성 (field.p에 따라)
    const tickInterval = 10;
    const ticks = [];
    for (let i = tickInterval; i < field.p; i += tickInterval) {
        if (i < field.p - 5) { // 마지막 숫자와 겹치지 않도록
            ticks.push(i);
        }
    }
    ticks.forEach(x => {
    ...
    ctx.fillText(field.p, ...pointToCtx(vals, field.p - 0.5, -yBodge, false));
    ctx.textAlign = 'right';
    const xBodge = 4 / vals.wScale;
    ctx.fillText('0', ...pointToCtx(vals, -xBodge, -yBodge, false));
    ctx.fillText(field.p, ...pointToCtx(vals, -xBodge, field.p - 0.5, false));
    ctx.fillText(Math.floor(field.p/2), ...pointToCtx(vals, -xBodge, field.p/2 - 0.5, false));
    ctx.restore();
};
...
function resetGraph(ctx, drawPoints) {
    ...
    // 곡선 매개변수가 변경되었는지 확인하여 캐시 무효화
    const currentParams = `${curve.curveA}_${curve.curveB}`;
    if (ctx['_curveParams'] && ctx['_curveParams'] !== currentParams) {
        ctx['resetState'] = null; // 캐시 무효화
    }
    ctx['_curveParams'] = currentParams;
...
/**
 * Promise wrapper for addPointsAnimation
 * @param ctx
 * @param nP {Number} the base-P multiple of P (for labeling)
 * @param P {Point} the current point 'P'
 * @param nQ {Number} the base-P multiple of Q (for labeling)
 ...
 * @param options {Object?} optional list of options
 * @return {Promise<{nR: Number, R: Point}>} Promise that resolves when animation completes
 */
function addPointsAnimationPromise(ctx, nP, P, nQ, Q, options) {
    return new Promise((resolve) => {
        const originalDrawDoneCb = options?.drawDoneCb;
        const modifiedOptions = {
            ...options,
            drawDoneCb: (nR, R) => {
                // 원래 콜백 호출
                if (originalDrawDoneCb) {
                    originalDrawDoneCb(nR, R);
                }
                // Promise resolve
                resolve({ nR, R });
            }
        ...
        const R = addPointsAnimation(ctx, nP, P, nQ, Q, modifiedOptions);
        // 즉시 반환되는 경우 처리 (Q === null 등)
        if (Q === null) {
            resolve({ nR: nP + nQ, R });
        }
    });
...
/**
 * 수동 동기화를 위한 단일 단계 애니메이션 (자동 진행 없음)
 * @param ctx {CanvasRenderingContext2D}
 * @param nQ {Number} current step number
 * @param Q {Point} current point
 * @param drawDoneCb {Function} callback when animation completes
 */
async function runManualAddPDemo(ctx, nQ, Q, drawDoneCb) {
    resetGraph(ctx, true);
    
    // 디버깅: 유한체 기본점 출력
    const baseP = curve.P();
    console.log(`[유한체 디버깅] 7번 곡선 기본점 P = (${baseP.x}, ${baseP.y})`);
    
    const result = await addPointsAnimation(ctx, 1, baseP, nQ, Q, {
        coords: true,
        labels: {1: curve.P()},
        drawDoneCb: (nR, R) => {
            // 디버깅: 유한체 좌표 출력
            if (R) {
                console.log(`[유한체 디버깅] ${nR}P = (${R.x}, ${R.y})`);
            }
            if (drawDoneCb) drawDoneCb(nR, R);
            // 자동 진행하지 않음 - 동기화 컨트롤러가 제어
        }
    });
...
export {
    ...
    runExchangeDemo,
    labelIdleGraph,
    addPointsAnimation,  // 동기화를 위해 export
    addPointsAnimationPromise,  // Promise 버전
};
