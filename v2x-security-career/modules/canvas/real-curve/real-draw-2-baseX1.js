import * as curve from './real-curve-2-modified.js';
import * as common from '../common.js';
const TWO_PI = 2 * Math.PI;

// x와 y 범위를 동일하게 설정하여 그래프 변형 방지
let dimensions = {xMin: -9, xMax: 21, yMin: -15, yMax: 15}; // 둘 다 30 범위
...
function preCalcValues(ctx) {
    const dotRadius = 3;
    // Use canvas logical size instead of getBoundingClientRect to avoid zoom issues
...
function plotNPs(ctx, vals, color, ...nVals) {
    ...
    nVals.sort(numSort).forEach(nToPlot => {
        ...
        // 정확한 픽셀 위치를 위해 반올림
        const roundedQ = { x: Q.x, y: Q.y };
        plotPoint(vals, nToPlot, roundedQ, color);
    });
}
 ...
 */
function addPoints(ctx, nP, P, nQ, Q, drawDoneCb, targetR = null) {
    const vals = preCalcValues(ctx);
    let start, prev;

    // 목표 점이 주어지면 그것을 사용, 아니면 계산
    const R = targetR || curve.add(P, Q);
...
function writeEquation(ctx, vals, np, nq, nr) {
    ...
    // 왼쪽 상단으로 위치 변경
    ctx.fillText(`${s(np)}P + ${s(nq)}P = ${s(nr)}P`, 20, 20);
    ctx.restore();
}
...
async function runAssocDemo(ctx, updateCb, drawDoneCb) {
    ...
    let next = async () => {
        ...
        const U = points[nU];
        const V = points[nV];
        const preCalculatedR = points[nU + nV]; // 미리 계산된 점
        // 미리 계산된 점을 목표로 선을 그림
        const R = await addPoints(ctx, nU, U, nV, V, (nR, targetR) => {
            // 콜백에서 정확한 미리 계산된 점을 다시 그림
            plotPoint(vals, nU + nV, preCalculatedR, 'red');
            if (drawDoneCb) drawDoneCb(nR, preCalculatedR);
            if (common.canvasIsScrolledIntoView(ctx.canvas)) {
