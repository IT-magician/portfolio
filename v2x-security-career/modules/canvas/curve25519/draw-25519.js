...
function preCalcValues(ctx) {
    ...
    const w = Math.floor(ctx.canvas.width / ratio);
    const h = Math.floor(ctx.canvas.height / ratio);
 ...
 * @param customDurations {Object?} optional custom animation durations
 */
function addP(ctx, n, Q, drawDoneCb, customDurations) {
    const vals = preCalcValues(ctx);
    resetGraph(ctx, vals);
    let start, prev;
    ...
    const duration = customDurations || {
        line: 3000,
        linePause: 0,
        negate: 800,
        done: 100,
 ...
 * @param customDurations {Object?} optional custom animation durations
 */
async function runDemo(ctx, updateCb, drawDoneCb, n, Q, customDurations) {
    ctx.save();
    ctx.fillStyle = 'white';
    ...
    let next = async () => {
        ...
        }, customDurations);  // Pass custom durations
